...
- bio::semialphabet requires your type to have a numerical representation (rank), as well as
  an alphabet size and comparison operators.
- bio::writable_semialphabet additionally requires being able to change the value of the object
  via the rank representation.
- bio::alphabet requires that your type has a visual representation in addition to the numerical representation.
  Usually this is a character type like char.
- bio::writable_alphabet additionally requires being able to change the value of the object
...
In this section we want to implement a bio::alphabet that consists of the characters `S` and `W` to represent
...
A look at the documentation of bio::alphabet will reveal that it is actually a refinement of other concepts,
more precisely bio::semialphabet which in turn refines std::copy_constructible and std::totally_ordered.
Let's check those:
...
As you can see from the `static_assert` our dna2 alphabet now models bio::semialphabet and
bio::writable_semialphabet:
...
Now that you have a feeling for concepts, have a look at bio::alphabet and bio::writable_alphabet and make
...
3. There is a function object that tells us which characters are valid for a given alphabet: bio::char_is_valid_for.
...
At this point the bio::alphabet concept should be modelled successfully and even bio::writable_alphabet
...
Often it is not required to implement the entire class yourself, instead you can derive from bio::alphabet_base
which defines most things for you if you provide certain conversion tables. Read the documentation of
bio::alphabet_base for details. This implementation deduces `bool` as the smallest possible rank type.
...
In that case, you need to specialise the bio::custom::alphabet class template and provide the required functionality
as static members.
...
Please note that for the overloads of bio::alphabet_size and bio::char_is_valid_for our alphabet type has to
...
With the overload of bio::char_is_valid_for we allow assignment to the underlying boolean type
