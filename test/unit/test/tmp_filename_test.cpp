...
{
    bio::test::tmp_filename t1{"aggr_test"};
    bio::test::tmp_filename t2("aggr_test");
...
{
    EXPECT_THROW(bio::test::tmp_filename t1{nullptr}, fs::filesystem_error);
    EXPECT_THROW(bio::test::tmp_filename t1(nullptr), fs::filesystem_error);
}
...
{
    bio::test::tmp_filename t1{"mv_ctr_test"};
    bio::test::tmp_filename t2{"mv_ctr_test"};
    bio::test::tmp_filename t3{std::move(t2)};
    EXPECT_NE(t1.get_path(), t3.get_path());
    bio::test::tmp_filename t4(std::move(t1));
...
TEST(tmp_filename_throw, throw)
{
    if (!system("man putenv > /dev/null 2>&1"))
    {
        ...
        putenv(str);
        EXPECT_THROW(bio::test::tmp_filename t1{"throw"}, std::filesystem::filesystem_error);
    }
}
