...
{
    EXPECT_EQ(bio::detail::sizeof_bits<int8_t>, 8);
    EXPECT_EQ(bio::detail::sizeof_bits<uint8_t>, 8);
    EXPECT_EQ(bio::detail::sizeof_bits<int16_t>, 16);
    EXPECT_EQ(bio::detail::sizeof_bits<uint16_t>, 16);
    EXPECT_EQ(bio::detail::sizeof_bits<int32_t>, 32);
    EXPECT_EQ(bio::detail::sizeof_bits<uint32_t>, 32);
    EXPECT_EQ(bio::detail::sizeof_bits<int64_t>, 64);
    EXPECT_EQ(bio::detail::sizeof_bits<uint64_t>, 64);
}

TEST(bit_manipulation, is_power_of_two)
{
    constexpr bool is_power_of_two0 = bio::detail::is_power_of_two(0);
    constexpr bool is_power_of_two1 = bio::detail::is_power_of_two(1);
    constexpr bool is_power_of_two2 = bio::detail::is_power_of_two(2);
    constexpr bool is_power_of_two3 = bio::detail::is_power_of_two(3);
    ...
    {
        EXPECT_TRUE(bio::detail::is_power_of_two(power_of_two));

        size_t next_power = (power_of_two << 1);
        for (size_t i = power_of_two + 1, k = 0; i < next_power && k < max_iterations; ++i, ++k)
        {
            EXPECT_FALSE(bio::detail::is_power_of_two(i)) << i << " should not be a power of two.";
        }
    }
}
...
{
    constexpr size_t next_power_of_two0 = bio::detail::next_power_of_two(0);
    constexpr size_t next_power_of_two1 = bio::detail::next_power_of_two(1);
    constexpr size_t next_power_of_two2 = bio::detail::next_power_of_two(2);
    constexpr size_t next_power_of_two3 = bio::detail::next_power_of_two(3);
    ...
    for (size_t power_of_two = 1; power_of_two <= (size_t{1u} << 31); power_of_two <<= 1)
    {
        EXPECT_EQ(bio::detail::next_power_of_two(power_of_two), power_of_two);
...
TYPED_TEST(unsigned_operations, most_significant_bit_set)
{
    ...
    constexpr size_t two1  = bio::detail::most_significant_bit_set<unsigned_t>(0b0101);
    constexpr size_t two2  = bio::detail::most_significant_bit_set<unsigned_t>(0b0111);
    constexpr size_t seven = bio::detail::most_significant_bit_set<unsigned_t>(0b10010010);
    EXPECT_EQ(zero, 0u);
    EXPECT_EQ(one1, 1u);
    EXPECT_EQ(one2, 1u);
    EXPECT_EQ(two1, 2u);
...
TYPED_TEST(unsigned_operations, count_leading_zeros)
{
    ...
    constexpr size_t t4 = bio::detail::count_leading_zeros<unsigned_t>(0b0110);
    constexpr size_t t5 = bio::detail::count_leading_zeros<unsigned_t>(0b0100);
    constexpr size_t t6 = bio::detail::count_leading_zeros<unsigned_t>(0b10100000);
    EXPECT_EQ(t1, bio::detail::sizeof_bits<unsigned_t> - 1u);
    EXPECT_EQ(t2, bio::detail::sizeof_bits<unsigned_t> - 3u);
    EXPECT_EQ(t3, bio::detail::sizeof_bits<unsigned_t> - 2u);
    EXPECT_EQ(t4, bio::detail::sizeof_bits<unsigned_t> - 3u);
    EXPECT_EQ(t5, bio::detail::sizeof_bits<unsigned_t> - 3u);
    EXPECT_EQ(t6, bio::detail::sizeof_bits<unsigned_t> - 8u);
...
{
    if (v & (unsigned_t{1u} << (bio::detail::sizeof_bits<unsigned_t> - 1)))
        return v;

    unsigned_t t = v | (v - 1);
    unsigned_t w = (t + 1) | (((~t & -~t) - 1) >> (bio::detail::count_trailing_zeros(v) + 1));
    return w;
}
...
TEST(to_little_endian, byte)
{
    ...
    EXPECT_EQ(bio::detail::to_little_endian(val), 0x01);
}
