...
TEST(view_persist, delegate_to_view_all)
{
    ...
    // pipe notation
    auto v = vec | bio::ranges::views::persist;
    EXPECT_RANGE_EQ("foo"sv, v);

    // function notation
    EXPECT_RANGE_EQ("foo"sv, bio::ranges::views::persist(vec));

    // combinability
    EXPECT_RANGE_EQ("fo"sv, vec | bio::ranges::views::persist | std::views::take(2));
    EXPECT_RANGE_EQ("of"sv, vec | std::views::reverse | bio::ranges::views::persist | std::views::drop(1));

    // store combined
    auto a1 = bio::ranges::views::persist | std::views::take(2);
...
TEST(view_persist, wrap_temporary)
{
    ...
    // pipe notation
    EXPECT_RANGE_EQ("foo"sv, std::string{"foo"} | bio::ranges::views::persist);

    // function notation
    EXPECT_RANGE_EQ("foo"sv, bio::ranges::views::persist(std::string{"foo"}));

    // combinability
    EXPECT_RANGE_EQ("fo"sv, std::string{"foo"} | bio::ranges::views::persist | std::views::take(2));
    EXPECT_RANGE_EQ("o"sv,
                    std::string{"foo"} | bio::ranges::views::persist |
...
TEST(view_persist, const)
{
    ...
    EXPECT_RANGE_EQ("foo"sv, t{"foo"} | bio::ranges::views::persist);

    // outer const
    auto const & v2 = std::string{"foo"} | bio::ranges::views::persist;
    EXPECT_RANGE_EQ("foo"sv, v2);

    // inner + outer const
    using t         = std::string const;
    auto const & v3 = t{"foo"} | bio::ranges::views::persist;
...
TEST(view_persist, concepts)
{
    ...
    EXPECT_TRUE(bio::ranges::const_iterable_range<decltype(std::string{"foo"})>);
    EXPECT_TRUE((std::ranges::output_range<decltype(std::string{"foo"}), char>));

    auto v1 = std::string{"foo"} | bio::ranges::views::persist;
