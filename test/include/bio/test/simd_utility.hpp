...
#include <bio/meta/simd/concept.hpp>

//!\cond DEV
/*!\brief #SIMD_EQ checks if the sizes and the content of two given
 * bio::simd::simd_type variables matches. It is like  #EXPECT_EQ, but for bio::simd::simd_type
 * types.
 * \ingroup simd
 * \param[in] left_simd_argument Left hand side operand of type bio::simd::simd_type.
 * \param[in] right_simd_argument Right hand side operand of type bio::simd::simd_type.
 *
 * \attention
 ...
 * ###Example
 *
 * \include test/snippet/meta/simd/simd_test_utility.cpp
 */
#define SIMD_EQ(...) do { \
    auto [left_simd_argument, right_simd_argument] = std::tuple{__VA_ARGS__}; \
    static_assert(bio::simd::simd_concept<decltype(left_simd_argument)>, "The left argument of SIMD_EQ is not a simd_type"); \
    static_assert(bio::simd::simd_concept<decltype(right_simd_argument)>, "The right argument of SIMD_EQ is not a simd_type"); \
    ...
    using _simd_traits_t = bio::simd::simd_traits<decltype(left_simd_argument)>; \
