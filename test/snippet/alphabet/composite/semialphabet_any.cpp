...
void algorithm(std::vector<bio::semialphabet_any<10> > & r, bool is_murphy)
{
    // Algorithm example that replaces rank 0 with rank 1
    for (auto & v : r)
        if (bio::to_rank(v) == 0)
            bio::assign_rank_to(1, v);

    // Here we reify the type for printing
    if (is_murphy)
        print(r | bio::views::convert<bio::aa10murphy>);
    else
        print(r | bio::views::convert<bio::aa10li>);
}
...
void algo_pre(bio::aa10li_vector const & v)
{
    std::vector<bio::semialphabet_any<10> > tmp = v | bio::views::convert<bio::semialphabet_any<10>>
                                                       | bio::views::to<std::vector>();
    algorithm(tmp, false);
}

void algo_pre(bio::aa10murphy_vector const & v)
{
    std::vector<bio::semialphabet_any<10> > tmp = v | bio::views::convert<bio::semialphabet_any<10>>
                                                       | bio::views::to<std::vector>();
    algorithm(tmp, true);
...
int main()
{
    bio::aa10li_vector v1{"AVRSTXOUB"_aa10li};
    algo_pre(v1); // BIKBBBKCB

    bio::aa10murphy_vector v2{"AVRSTXOUB"_aa10murphy};
