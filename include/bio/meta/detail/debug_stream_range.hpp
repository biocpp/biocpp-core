...
{
/*!\brief A helper concept definition for ranges that can be streamed to the bio::debug_stream.
 ...
 * \{
 */
/*!\brief All input ranges can be printed to the bio::debug_stream element-wise (if their elements are printable).
 ...
 * \details
 *
 * If the element type models bio::alphabet (and is not an unsigned integer), the range is printed
 ...
 *
 * \if DEV
 * Note that overloads for range based streaming need to refine the bio::detail::debug_stream_range_guard concept
